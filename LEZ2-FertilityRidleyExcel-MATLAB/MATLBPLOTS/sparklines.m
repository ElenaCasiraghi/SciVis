...
% IEEE Access https://ieeexplore.ieee.org/document/8981975
    ...
    % Per accedere ai nomi dei campioni accedo all'attributo Properties.RowNames
    % Per accedere ai nomi delle Variabili accedo all'attributo Properties.VariableNames
    
    numRows = size(data,1);
    figure('Name', ['Small Multiple'], 'units', 'normalized', 'Position', [0.01 0.01 0.75 0.8]);
    numR = 3; numC = round(numRows/3);
    for nNz = 1:numRows
        hold on; subplot(numR,numC, nNz); 
        if strcmpi(data.Properties.RowNames(nNz),'Africa')
            h=plot(table2array(data(nNz,:)), '-r');
        else; plot(table2array(data(nNz,:)), '-'); end
        
        ax = gca;
        ax.YLim = [0, max(max(table2array(data(1:8,:))))+0.5];
        
        [minV, indMin] = min(table2array(data(nNz,:)));
        [maxV, indMax] = max(table2array(data(nNz,:)));
        
        ax.YTick = [minV maxV];
        ax.YTickLabel = {num2str(minV), num2str(maxV)};
        
        ax.XTickLabel = data.Properties.VariableNames(sort([indMax, indMin]));
        ax.XTick = sort([indMax ,indMin]);
        ax.XTickLabelRotation=45;
        
        title(data.Properties.RowNames(nNz));
    end
    ...
    numRows = size(data,1);
    figure('Name', ['Stripplot'], 'units', 'normalized', 'Position', [0.01 0.01 0.75 0.8]);
    numR = 3; numC = round(numRows/3);
    ...
    for nNz = 1:numRows
        hold on; 
        if strcmpi(data.Properties.RowNames(nNz),'Italy') || contains(data.Properties.RowNames(nNz),'Europe')
            plot(table2array(data(nNz,:)), '-*r');
        elseif contains(data.Properties.RowNames, 'America'); plot(table2array(data(nNz,:)), '*', 'MarkerFaceColor', []); 
        elseif contains(data.Properties.RowNames, 'Africa');  plot(table2array(data(nNz,:)), '*', 'MarkerFaceColor', [254,240,217]/255);
        else; end
...
%         title(data.Properties.RowNames(nNz));
     ...
     legend(data.Properties.RowNames);
    ...
    for nC = 1: numel(countries)
        ...
        if strcmpi(countries{nC}, 'Italy') 
            h(nC).LineStyle = ':'; h(nC).Color = colors(index, :)/255; 
